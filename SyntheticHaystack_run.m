function SyntheticHaystack_run(run_mode)
...
switch run_mode
    ...
    case "default"
        ...
        warning('SyntheticHaystack:ExpensiveRunMode', ...
            'Default mode preserves the imported high-cost settings and may take a long time on laptops. Use SyntheticHaystack_run("smoke") for a quick run.');
    case "full"
        mode_sbp_dx = 0.0125;
        mode_f_s = 480000;
        mode_receive_length = 100e-3;
        ...
        mode_diagnostic_stride = 500;
        warning('SyntheticHaystack:ExpensiveRunMode', ...
            'Full mode uses the imported high-cost settings and may take a long time on laptops. Use SyntheticHaystack_run("smoke") for a quick run.');
end
...
for ind_sbp_x = 1:length(sbp_x)
    ...
    if mode_show_diagnostic_plots && ismember(ind_sbp_x, 1:mode_diagnostic_stride:length(sbp_x))
        ...
        plot(receive_t, real(sum(res_waves,1)))
        xlabel('time [s]')
